...
0. **Freshness check.** Before touching the triggering issue at all: confirm this project is running the latest DevAudit-Installer templates, so the rest of this run isn't driven by stale scripts/hooks/skills.
   ...
   - **Systemic-anomaly escalation (devaudit-installer#867).** This step only checks *this* project's version staleness. If a `devaudit update` here (or any framework file's behavior) looks wrong in a way that suggests every consumer would hit the same thing — not just "this one is behind" — suggest to the operator that they run the operator-only `fleet-doctor` skill (in `DevAudit-Installer`, not synced to consumers) for a fleet-wide view before continuing. This is advisory only: never block or auto-invoke it — a single-issue run should never pay the cost of auditing every other consumer.
...
5b. **E2E gate verification — mandatory before commit (devaudit-installer#226).** After running gates in step 5, verify the E2E gate actually ran before proceeding to step 7 (commit). This is the skill-level enforcement that backs the pre-push hook.
   ...
   **Plan ↔ spec cross-check (devaudit-installer#578).** A plan can name Playwright as the verification method for a surface and ship zero of it — neither the AC-consistency check above nor the pre-push freshness check notices, because both answer "did *a* run happen," not "does evidence exist for *this* REQ's planned coverage." This is exactly what happened on `wawagardenbar-app` REQ-095: `implementation-plan.md` scoped Playwright coverage for four surfaces, only Vitest was written, and the push still went through on a stale unrelated regression run. Before running E2E:

   - Read `compliance/plans/REQ-XXX/implementation-plan.md`'s test-strategy / surface-inventory section. Collect every surface that names "Playwright" (or "E2E") as its verification method.
   - For each such surface, check whether any `e2e/**/*.spec.ts` file tags this REQ (`tagTest('REQ-XXX', ...)` or `@requirement REQ-XXX`) covering that surface.
   - If the plan names Playwright for a surface and no spec tags this REQ at all, **HALT**: "Plan requires Playwright coverage for REQ-XXX (surface: <surface>); no spec tags this REQ (`tagTest('REQ-XXX', …)` / `@requirement REQ-XXX`). Invoke `e2e-test-engineer` to write the planned coverage, or amend the plan (Phase 1 step 5b drift management) if the surface genuinely doesn't need it — do not silently ship the gap."
   - If the plan names no Playwright coverage anywhere, this check is a no-op — proceed to the sentinel check above.
