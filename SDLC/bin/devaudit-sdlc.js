...
if (!phase) {
  console.error(
    '❌ ERROR: Missing required configuration property. Execution syntax: devaudit-sdlc --phase=<1-5|issue>'
  );
  process.exit(1);
}
...
if (!targetBlueprint) {
  console.error(
    `❌ ERROR: Invalid phase argument [${phase}]. Supported options: 1, 2, 3, 4, 5, issue`
  );
  process.exit(1);
}
...
if (viewOnly) {
  if (!fs.existsSync(blueprintPath)) {
    console.error(
      '❌ ERROR: Target operational asset blueprint could not be resolved.'
    );
    process.exit(1);
  }
...
try {
  ...
  if (fs.existsSync(sentinelPath)) {
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        console.warn(
          '⚠️ WARNING: Sentinel file contained a legacy single-object format. Migrating to array format.'
        );
        phaseHistory = [parsed];
      }
    } catch (parseError) {
      ...
      console.warn(
        '⚠️ WARNING: Sentinel file was corrupt or in legacy plain-text format. Overwriting with fresh phase history.'
      );
      phaseHistory = [];
    }
  }
  ...
  console.log(
    `🚀 Phase ${phase} orchestration active. Your local git commit gates are now open.`
  );
  console.log(
    `📋 Phase history: ${phaseHistory.map((r) => r.currentPhase).join(' → ')}`
  );
  ...
  console.error(
    '❌ SYSTEM ERROR: Failed to instantiate workspace sentinel tracking state:',
    error.message
  );
  process.exit(1);
}
