...
| REQ-050 | #175  | HIGH   | compliance/evidence/REQ-050/ | TESTED - PENDING SIGN-OFF (Stage 3 complete 2026-05-28; PR #177 merged to develop `4f0cbeb`; full evidence pack; 50 vitest cases pass [17 new + 33 updated regression], full suite 875 pass; tsc + eslint + semgrep + npm-audit clean; e2e n/a; CI release attribution clean via `[REQ-050]` PR title). Expense-restock stock-leak fix for `trackByLocation` inventory (#117 P0 follow-up; closes the gap REQ-044 left in the expense path). New `applyExpenseStockDelta` helper + 3-site refactor in `services/expense-inventory-link-service.ts` (apply, reverse, runReversalPass): doc-mutation + save so the pre-save hook recomputes `currentStock = sum(locations)` correctly. Ships with `scripts/reconcile-track-by-location-stock.ts` for data-repair of pre-existing drifted rows. UAT row Orijin-Small `694540785172c0ca02759606` manually repaired 2026-05-28 (Main Store 0 → 48).  | ostendo-io      | 2026-05-28 |
