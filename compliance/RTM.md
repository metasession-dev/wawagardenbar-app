...
| REQ-051 | #196  | HIGH   | compliance/evidence/REQ-051/ | PLANNING (DFR aggregation: `FinancialReportService.generateDailySummary(date)` switches from `startOfDayWAT(date)`/`endOfDayWAT(date)` [calendar-day range] to a new `businessDayRange(date, cutoff)` helper [current-business-day range]. Root cause traced via the #196 e2e triage: cutoff defaults to 15:00 WAT, so any DFR opened before 15:00 WAT showed ₦0.00 — every CI run hit this 100% of the time because GHA runners are UTC and tests land at ~07:00 WAT. Scope tight: 2 LOC in `services/financial-report-service.ts` + new ~10 LOC helper in `lib/business-date.ts`. `generateDateRangeReport` deliberately out of scope. Unblocks 7 e2e tests in #160/#158/#195.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | ostendo-io      | 2026-05-30 |
