...
| REQ-050 | #175  | HIGH   | compliance/evidence/REQ-050/ | IN PROGRESS — Stage 1 (plan written, awaiting HIGH plan-approval checkpoint). Expense-restock stock-leak fix for `trackByLocation` inventory (#117 P0 follow-up; closes the gap REQ-044 left in the expense path). Mirror REQ-044's `applyOrderStockDelta` pattern in `services/expense-inventory-link-service.ts` (apply, reverse, runReversalPass): doc-mutation + save so the pre-save hook recomputes `currentStock = sum(locations)` correctly. Ships with `scripts/reconcile-track-by-location-stock.ts` for data-repair of pre-existing drifted rows. UAT row Orijin-Small `694540785172c0ca02759606` manually repaired 2026-05-28 (Main Store 0 → 48).                                                                                                                                                                                                                                  | ostendo-io      | 2026-05-28 |
