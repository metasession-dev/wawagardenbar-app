...
| REQ-049 | #166  | HIGH   | compliance/evidence/REQ-049/ | IN PROGRESS — Stage 1 (plan written, awaiting HIGH plan-approval checkpoint). Webhook idempotency guard (#117 P0 #1): new `ProcessedWebhookEvent` model with unique `(provider, eventId)` index + `lib/webhook-idempotency.ts` helper; wired into `app/api/webhooks/{paystack,monnify}/route.ts` after signature verification, before any side-effect. Closes the replay → re-issue-rewards value leak.                                                                                                                                                                                                                                                                           | ostendo-io      | 2026-05-28 |
