...
| RELEASED | #166  | HIGH   | compliance/evidence/REQ-049/ | TESTED - PENDING SIGN-OFF (Stage 3 complete 2026-05-28; PR #167 merged to develop `2a1dac8`; full evidence pack — test-scope/plan/execution-summary/security-summary + STRIDE/ai-use-note + prompts/implementation-plan + release ticket; 12 new unit + integration tests green, full suite 858 pass; tsc + eslint + semgrep + npm-audit clean on REQ-049 code; e2e n/a — no UI surface; CI release attribution clean via `[REQ-049]` PR title convention). Webhook idempotency guard (#117 P0 #1): new `ProcessedWebhookEvent` model with compound unique `(provider, eventId)` index + `lib/webhook-idempotency.ts` helper using MongoDB E11000 for race-safe dedup; wired into `app/api/webhooks/{paystack,monnify}/route.ts` after signature verification, before any side-effect. Closes the replay → re-issue-rewards value leak. | ostendo-io      | 2026-05-28 |
