...
| D4  | Newly-created kitchen-role user redirected to `/unauthorized` on UAT instead of reaching `/dashboard/kitchen/recipes`. Root cause: the parent `/dashboard/layout.tsx` gates with `requireAdmin()` whose allowlist (`csr / admin / super-admin / bar / waiting`) does not include `kitchen`. The step-11 child `requireKitchen()` layout never got a chance to run. Step-11 oversight — I changed the kitchen sub-route's gate but didn't notice the parent dashboard layout was already excluding kitchen. | 2026-05-13 | New `requireDashboardAccess()` helper in `lib/auth-middleware.ts` allows the full dashboard-capable role set (csr / admin / super-admin / bar / waiting / kitchen); `/dashboard/layout.tsx` switched to it. Sub-routes (orders / finance / inventory / etc.) continue to deny kitchen via their existing `requirePermission(...)` gates (which internally call `requireAdmin` — still excludes kitchen). `/dashboard/page.tsx` gains a kitchen→`/dashboard/kitchen/recipes` redirect so kitchen role lands at its home rather than rendering the inaccessible overview. Bar/waiting also funnelled to `/dashboard/orders` since they're csr-equivalent. tsc 0, 718 vitest still pass. |
