...
- `e2e/kitchen/daily-report-regression.spec.ts` — Step 8 (DFR regression)
- `e2e/kitchen/recipe-and-production.spec.ts` — original surface smoke

Steps 1–8 are no longer manual UAT work — they run on every CI green of
...
| DFR renders with Total Revenue card                   | `daily-report-regression.spec.ts` |
| AC14 — production events do not move revenue          | `daily-report-regression.spec.ts` |

> The third historical Step-8 item — "Per-portion COGS uses weighted-average
> from InventoryItemCostHistory" — has no labelled cell on the DFR client,
> so a UI assertion would only test markup. The math is the load-bearing
> property, fully covered by
> `__tests__/lib/expense-inventory-link.test.ts` (`computeWeightedAverageCost`,
> 5 tests) and `__tests__/services/expense-inventory-link.test.ts`.
