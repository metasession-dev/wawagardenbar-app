# UAT Checklist — REQ-037 (Edit + delete kitchen ingredients)

**Requirement:** Edit + delete kitchen ingredients with safe-removal guard
...
- [ ] Author a kitchen ingredient, record a few StockMovements against it (via expense link), then **delete** the ingredient.
...
| Delete confirmation dialog renders                                    | `inventory-crud.spec.ts` AC3     |
| Delete blocked by active recipe → error names recipe                  | `inventory-crud.spec.ts` AC3     |
| Deactivate recipe → retry delete succeeds                             | `inventory-crud.spec.ts` AC3     |
