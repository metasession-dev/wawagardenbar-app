...
**Fix (this PR):**
- Reused the existing `optionalCount` preprocessor (`"" | null | undefined → undefined`) for `maxRedemptionsPerUser`, so blank parses as unlimited (`undefined`) while genuine values stay positive integers.
- Added a `maxRedemptionsPerUser optional (REQ-046 D5)` describe block to `__tests__/components/reward-rule-form-schema.test.ts` (3 cases): blank `""` → undefined; `null` → undefined; `'5'` → 5 and `'0'` rejected.
