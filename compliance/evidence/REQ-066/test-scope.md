...
- **AC9 — chokepoint return shape:** `__tests__/services/order-service.completeOrder.test.ts` — 2 new cases (deductionFailed=true + deductionError set when applyOrderStockDelta throws; both undefined on the happy path). Used by `updateOrderStatusAction` to add `warning` to its `ActionResult` for the UI toast.
...
- `e2e/admin-order-inventory-delta.over-sell.spec.ts` (AC9): **force-mutates** chiller1 down to 2 units + sets defaultSalesLocation, seeds an order for 3 units, advances via kitchen-display. Asserts status=completed + inventoryDeducted=false + `inventory_deduction_failed` IncidentEvent written + locations unchanged + zero stockmovement rows. The operator-reported scenario from 2026-06-04 ("nothing happened on the kitchen display") is now pinned in CI.
...
- **No pre-sale availability check** — filed as separate follow-up REQ-067 ([wawagardenbar-app #286](https://github.com/metasession-dev/wawagardenbar-app/issues/286)). Express order + Quick Actions currently skip the check entirely; the existing `isAvailableForOrder` is aggregate-based, not sale-point-aware. AC9's IncidentEvent + UI warning is the backstop until REQ-067 ships. AC1 + AC8 ensure over-sells never cause data corruption — only operator-facing UX is affected.
