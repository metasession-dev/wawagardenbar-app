...
| AC3  | Sub-category selection shows only matching available sellable items.                    | E2E item assertions plus unit/integration test for `expressSearchMenuAction({ mainCategory, category })`.                              |
...
| AC6  | Menu management list filters main -> sub -> matching rows.                              | E2E or component-level coverage on `/dashboard/menu`; assert initial main step and narrowed rows.                                      |
...
| AC8  | Sellable inventory filtering starts main -> sub.                                        | E2E or component-level coverage on `/dashboard/inventory` sellable tab.                                                                |
...
- Focused E2E coverage for express order cascade/back-navigation/cross-main cart preservation.
- E2E or component/unit coverage for at least one admin management surface; target `/dashboard/menu` first because it is the main admin item-management surface.
- Add sellable inventory coverage if implementation risk is not sufficiently pinned by shared component/helper tests.
