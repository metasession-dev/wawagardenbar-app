...
| AC2 | **Amended 2026-09-05** — Given a super-admin opens `/dashboard/menu` and clicks the "Pricing Windows" button, When they land on the dedicated `/dashboard/menu/pricing-windows` page and view the "Show Price Window" and "Happy Hour Window" sections, Then they can independently enable/disable each window and set its daily start/end time, and saving persists both without affecting Settings' Business Hours section. (Originally specified as a Settings tab; relocated per operator request after initial UAT review — see amendment note below.) | REQ-MENUMGT-008/009 (new — see amendment)                                                                                                                                         |
...
- **In scope:**
  ...
  - ~~`components/features/admin/settings-form.tsx` — add "Show Price Window" and "Happy Hour Window" sections~~ **Superseded 2026-09-05**: these sections were relocated to a new `components/features/admin/pricing-windows-form.tsx` rendered by a new `app/dashboard/menu/pricing-windows/page.tsx`, reached via a "Pricing Windows" button on `/dashboard/menu`. `settings-form.tsx` no longer has a Pricing Windows tab. The underlying `showPriceWindow`/`happyHourWindow` schema fields, `SettingsService` methods, and `/api/settings` contract are unchanged.
...
| Pricing windows — dedicated page (relocated 2026-09-05) | `/dashboard/menu/pricing-windows` — `components/features/admin/pricing-windows-form.tsx` | In scope                                                                             |
...
- **Spec(s):**
  ...
  - `e2e/admin/pricing-windows.spec.ts` (AC2 — supersedes deleted `e2e/settings/pricing-windows.spec.ts`, see amendment)
...
## Amendment — 2026-09-05: Pricing Windows relocated out of Settings

Operator request after initial UAT review of PR #698: move the Show Price
Window / Happy Hour Window controls off the Settings page onto a
dedicated page at `/dashboard/menu/pricing-windows`, reached via a new
"Pricing Windows" button next to "Edit All" on `/dashboard/menu`.

- **AC2** re-scoped as above (§2 table updated in place).
- **No change** to the data model (`showPriceWindow`/`happyHourWindow` on
  `models/settings-model.ts`), `SettingsService` resolution logic, ADR-004,
  or the `/api/settings` contract — only the admin UI's location moved.
- `components/features/admin/settings-form.tsx`: Pricing Windows tab and
  its zod schema fields removed; `TabsList` reverted `grid-cols-7` →
  `grid-cols-6`.
- New: `app/dashboard/menu/pricing-windows/page.tsx`,
  `components/features/admin/pricing-windows-form.tsx`.
- `e2e/settings/pricing-windows.spec.ts` deleted (route no longer exists);
  superseded by `e2e/admin/pricing-windows.spec.ts`.
- Bundled in the same amendment: fixed a live UAT crash on
  `/dashboard/menu/edit-all` (risk R-025 materializing — legacy menu items
  missing `showPrice`/`happyHourPrice`) via `?? price` fallbacks at every
  read site.
- Delivered on `fix/REQ-102-edit-all-selector-collision` → PR #700.
- Tracked via issue #696 amendment comment.
