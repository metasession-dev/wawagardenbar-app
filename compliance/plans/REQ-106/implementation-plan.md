...
**Gap:** AC1 said the Current Cash Position section "always renders" but never specified _which_ position — the implementation computed it relative to whatever date/range the Daily Report's picker was scoped to, so viewing a past date or a multi-day range showed that period's closing balance (or, for a range, the sum of movements across the whole range) rather than the actual present-moment till balance. A section titled "Current" reasonably implies real-time, not period-relative — flagged by the operator during UAT as reading like "an accumulation of all the cash over the report period" rather than "current cash position right now." Separately, the operator noted the aggregate four-line breakdown gives no way to verify which actual transactions (expenses, deposits, manual edits) produced the number.

**Resolution:** Amended AC9 (always show the live "now" position on the Daily Report, decoupled from the date picker) and AC10 (a dedicated `/dashboard/finance/cash-position` page showing the live balance plus an itemized, reverse-chronological ledger of every transferred cash-method expense, transferred cash deposit, and manual adjustment — cash sales shown as a single running total since opening rather than itemized per-transaction, since individual sale-level detail already belongs to the Daily/Range Report's own revenue breakdown). Folded into REQ-106 rather than a new REQ, per the operator's explicit direction, since this corrects REQ-106's own AC1 rather than adding an unrelated capability.
