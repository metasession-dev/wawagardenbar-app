...
**Opened:** 2026-08-03
**Owner:** WGB maintainer
**Review due:** Before REQ-098 plan approval and again at production review.
...
1. Server-side RBAC gate identical to `deleteTabAction` (admin/super-admin only) — never trust client-side UI gating alone.
2. `reason` is a required field at both the service-method signature and the UI dialog — no silent/blank write-off.
3. Every write-off writes an audit-log entry (`tab.write_off`) recording the acting user, reason, and amount — the same append-only audit log already used for `tab.delete`/`order.cancel`.
...
**Residual risk:** Pending implementation and evidence from the controls above. Structurally, this control set matches an existing shipped pattern (`deleteTabAction`'s RBAC gate) and the existing audit-log convention; it does not add reconciliation against write-off volume/frequency by staff member — worth a follow-up issue if abuse patterns are ever suspected (out of scope for REQ-098, which is additive-only).
...
4. The script is idempotent on rerun — a tab already `'written-off'` is skipped, not re-processed, so an accidental second invocation is a no-op rather than a double-write-off.
5. Dry-run test against a seeded fixture matching the 51-order profile (issue AC9) proves the selection criteria before any real invocation.

**Residual risk:** Pending implementation and evidence from the controls above. No invocation of the confirmed-write mode may occur against production before the dry-run output has been reviewed by the operator against the known 51-tab list from the original incident.
...
1. `scanDormantOpenTabs` dedups per 24h window via the existing `IncidentEventService.dedupRecent` mechanism — identical to `scanStalePaidOrders`'s already-shipped pattern.
...
2. Unit test proves the refusal fires on a second invocation and the original `writeOff` subdocument is unmodified.
3. The remediation script (R-020) treats "already written off" as a skip condition, not an error or a re-write, on rerun.
