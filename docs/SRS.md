...
#### REQ-REPORT-007 — Current Cash Position section · **Should** · regression

**Source:** `services/cash-position-service.ts`; `components/features/reports/cash-position-section.tsx`; `models/cash-position-adjustment-model.ts`; cross-ref REQ-FIN-003 (payment method), REQ-FIN-008 (cash deposits)

**Behaviour:** The Daily Report shows a system-computed running till balance — opening position (carried forward from a super-admin-entered opening balance) + cash sales in − cash-method transferred expenses out − transferred cash deposits out = closing position. Computed on demand from source collections (no persisted running counter), mirroring how `paymentBreakdown`/`tipsBreakdown`/`operatingExpenses` are already computed fresh per request. Cash tips are excluded — only sale revenue (`paymentBreakdown.cash`) counts as cash in.

- **Given** the Daily Report, **When** it renders for any date, **Then** a "Current Cash Position" section is always shown — including a defined empty state ("Opening balance not yet set") before any opening balance has been recorded, never omitted entirely.
- **Given** a cash-method pending expense group is approved and transferred, **When** the Daily Report is viewed for that transfer date, **Then** Current Cash Position reflects the deduction.
- **Given** a transfer-method pending expense group is approved and transferred, **When** the Daily Report is viewed, **Then** Current Cash Position is unaffected — only cash-method expenses touch the till.
- **Given** a super-admin, **When** they record a cash position adjustment (the initial opening balance or a later correction), **Then** the position updates immediately and the adjustment appears in an audit-trail list showing who, when, the amount, and an optional reason.
- **Given** a report date before any opening balance was set, **When** the Daily Report renders, **Then** the section explicitly states the position is not yet tracked for that date, never a misleading ₦0.
- **Given** a date-range report, **When** it renders, **Then** the section shows opening-of-range → closing-of-range only (no per-day waterfall breakdown).
