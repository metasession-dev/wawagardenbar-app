...
| REQ-MENU-008     | Displayed menu price matches whichever price would actually be charged right now            | Should   | regression | `components/features/menu/menu-item.tsx`; `menu-item-detail-modal.tsx`; `app/api/public/menu/route.ts`; REQ-102       |
...
| REQ-CHECKOUT-010 | Customer checkout shows only Monnify gateway options (no manual cash/transfer/card)         | Must     | regression | `components/features/checkout/checkout-form.tsx`; REQ-084                                                             |
| REQ-CHECKOUT-011 | Anonymous user can add to cart and reach checkout without login redirect                    | Must     | smoke      | `app/menu/page.tsx`, `menu-item-detail-modal.tsx`, `app/checkout/page.tsx`; REQ-084                                   |
...
| REQ-ORDER-006    | Time-window price resolution at order creation (happy-hour > show > default; override wins) | Must     | regression | `lib/order-line-totals.ts`; `services/settings-service.ts`; REQ-102                                                   |
...
| REQ-REPORT-007   | Daily report: Current Cash Position section + super-admin adjustment                        | Should   | regression | `services/cash-position-service.ts`; `components/features/reports/cash-position-section.tsx`; REQ-FIN-003/REQ-FIN-008 |
...
| REQ-RT-003       | Order-updated payload carries top-level status; terminal orders removed from store          | Must     | regression | `lib/socket-emit-helper.ts`, `kitchen-order-grid.tsx`, `order-queue.tsx`; REQ-083                                     |
...
| REQ-WA-001       | OTP delivery via WhatsApp (authentication category, no consent gate)                        | Must     | regression | `lib/notification-templates.ts`; `services/notification-service.ts`; REQ-053/054                                      |
| REQ-WA-002       | Transactional WhatsApp templates (order confirmation, status update, receipt, payment)      | Should   | regression | `lib/notification-templates.ts`; `services/notification-service.ts`; REQ-054/062                                      |
| REQ-WA-003       | Support reply via WhatsApp (transactional, falls through to email until Meta approval)      | Should   | regression | `lib/notification-templates.ts`; `services/notification-service.ts`; REQ-064                                          |
| REQ-WA-004       | Marketing WhatsApp templates (reward earned, expiring soon) with consent gate               | Should   | regression | `lib/notification-templates.ts`; `services/notification-service.ts`; REQ-063                                          |
| REQ-WA-005       | Inbound WhatsApp message handling (welcome, opt-out, support routing)                       | Should   | regression | `services/whatsapp-inbound-service.ts`; `app/api/webhooks/whatsapp/route.ts`; REQ-056                                 |
| REQ-WA-006       | Consent model: transactional default on, marketing default off                              | Must     | regression | `services/notification-service.ts`; `lib/notification-templates.ts`; REQ-053/063                                      |
...
#### REQ-MENU-008 — Displayed menu price matches whichever price would actually be charged right now · **Should** · regression

**Source:** `components/features/menu/menu-item.tsx`, `components/features/menu/menu-item-detail-modal.tsx`, `app/api/public/menu/route.ts`, `app/api/public/menu/[itemId]/route.ts`; cross-ref REQ-ORDER-006, REQ-102.
**Behaviour:** The public menu's item price display is server-resolved (never computed client-side), using the same precedence as REQ-ORDER-006 (happy-hour > show > default), so what a customer sees always matches what they'd be charged.

- **Given** the happy-hour or show-price window is active, **When** a customer views the menu list or an item's detail modal, **Then** the displayed price is the currently-active resolved price (`happyHourPrice`/`showPrice`), not the default `price`.
- **Given** neither window is active, **When** a customer views the menu, **Then** the displayed price is the default `price`, unchanged from pre-REQ-102 behaviour.
...
#### REQ-CHECKOUT-010 — Customer checkout: Monnify-only gateway · **Must** · regression

**Source:** `components/features/checkout/checkout-form.tsx`; cross-ref REQ-084.
**Behaviour:** The customer checkout path (`/checkout`) shows only Monnify payment options (Pay with Card, Pay with Transfer, Pay with USSD, Pay with Phone Number). Manual payment methods (cash, transfer, card) are not offered to customers. Admin-only payment recording remains available via the admin tab checkout form and order management actions.

- **Given** a customer at `/checkout`, **When** the payment step renders, **Then** only Monnify gateway options are visible.
- **Given** the customer checkout submission, **When** the order is created, **Then** no admin price override or manual payment fields are present in the request.

#### REQ-CHECKOUT-011 — Anonymous browsing: no login gate on add-to-cart · **Must** · smoke

**Source:** `app/menu/page.tsx`, `components/features/menu/menu-item-detail-modal.tsx`, `app/checkout/page.tsx`; cross-ref REQ-084.
**Behaviour:** An unauthenticated visitor can browse the menu, open an item detail modal, add an item to the cart, and navigate to `/checkout` without being redirected to login. A "Continue as Guest" banner is visible on the checkout page for unauthenticated users.

- **Given** an anonymous visitor on `/menu`, **When** they click an item and add it to the cart, **Then** the cart count updates and no login redirect occurs.
- **Given** an anonymous visitor with items in the cart, **When** they navigate to `/checkout`, **Then** the checkout page renders with a "Continue as Guest" banner.
...
- **Given** neither window is active, **When** an order is created, **Then** the charged line price is the item's `price` (default) — unchanged from pre-REQ-102 behaviour.
- **Given** a line carries an admin-supplied `priceOverride` on an item with `allowManualPriceOverride: true`, **When** an order is created regardless of window state, **Then** the override price is charged, not the window-resolved price.
...
#### REQ-ORDMGT-008 — Express order progressive category display · **Should** · regression

**Source:** `app/dashboard/orders/express/create-order/page.tsx`, `app/actions/admin/express-actions.ts`; cross-ref REQ-082.
**Behaviour:** Express order item selection uses progressive disclosure: all sellable `kind:'menu-item'` items are visible on landing, grouped by main category then sub-category. Main categories appear as toggle filter buttons; selecting one filters items to that category while preserving the grouped layout. Selecting a sub-category further filters items, shown flat. Search always filters items (not categories), scoped to the selected category or all items if no category is selected. Breadcrumb navigation shows the current category path (All Categories > Main > Sub) with a single back action. The cart/order context is preserved while staff navigate between categories, so one order can include items from multiple mains.

- **Given** a staff member opens `/dashboard/orders/express/create-order`, **When** the page first renders, **Then** all available menu items are shown grouped by main category and sub-category, with main category toggle buttons visible.
- **Given** no main category is selected, **When** staff selects a main category, **Then** items filter to that main category while remaining grouped by sub-category.
- **Given** a selected main category, **When** staff selects a sub-category, **Then** the item grid shows only available menu items whose `mainCategory` and `category` match the current selections, shown flat.
- **Given** any category selection state, **When** staff types into search, **Then** only matching items are shown (search filters items, not categories), scoped to the selected category or all items if no category is selected.
- **Given** staff has already added items to the cart, **When** they navigate back to All Categories or a different main category and add an item from another main, **Then** the existing cart lines remain and the new item is added to the same order context.

#### REQ-ORDMGT-009 — Express create order: order type + customer info · **Must** · regression

**Source:** `app/dashboard/orders/express/create-order/page.tsx`, `app/actions/admin/express-actions.ts`; cross-ref REQ-084.
**Behaviour:** The Express Create Order form includes an order type selector (dine-in, pickup, delivery). When pickup is selected, a preferred pickup time field appears and is required. When delivery is selected, delivery address fields (street, city, state, postal code) appear and are required. Customer information fields (name, email, phone) appear and are required for pickup and delivery orders. `SettingsService.calculateOrderTotals` is called with the correct `orderType` so delivery fees are applied for delivery orders.
...
#### REQ-ORDMGT-017 — Order completion auto-cash-mark scoped to non-tab orders · **Must** · regression
...
**Source:** `components/features/admin/admin-tab-checkout-form.tsx`; cross-ref REQ-084.
**Behaviour:** When staff close a tab from the dashboard, the admin tab checkout form renders with manual payment options (cash, transfer, card) — not Monnify gateway redirects. The form records the payment method, closes the tab, and marks all attached orders as paid. No Monnify checkout URL is returned.
...
- **Given** the same menu item is sold at more than one price within the selected date range (e.g. a half-portion line and a full-portion line, or a mid-range menu price change), **When** the report renders, **Then** the item's "line total" and the category's totalRevenue equal the sum of each order line's own price × quantity — never a single price (whichever line was encountered first) multiplied by the combined quantity across all lines. The same per-line summation applies to the costs table's costPerUnit/line total (REQ-100).
- **Given** the same menu item is sold at more than one portion size (`portionSize`: full/half/quarter) within the selected date range, **When** the report renders, **Then** each portion size appears as its own revenue/costs row with the correct per-portion unit price/costPerUnit and quantity, never blended into one row with a weighted-average price that doesn't match any actual sale price (REQ-101).
...
#### REQ-MENUMGT-007 — Menu management progressive category display · **Should** · regression

**Source:** `components/features/admin/menu-items-client.tsx`, `components/features/admin/menu-item-form.tsx`, `components/features/admin/menu-item-edit-form.tsx`; cross-ref REQ-082.
**Behaviour:** Admin menu-management item browsing uses progressive disclosure: all menu items are visible on landing, grouped by main category then sub-category. Main categories appear as toggle filter buttons; selecting one filters items to that category while preserving the grouped layout. Selecting a sub-category further filters items, shown flat. Search always filters items (not categories), scoped to the selected category or all items if no category is selected. Breadcrumb navigation shows the current category path. Create and edit forms keep the existing main-category-before-sub-category flow and clear an invalid sub-category value when the selected main category changes.

- **Given** a super-admin opens `/dashboard/menu`, **When** the list renders, **Then** all menu items are shown grouped by main category and sub-category, with main category toggle buttons visible.
- **Given** no main category is selected, **When** the super-admin selects a main category, **Then** items filter to that main category while remaining grouped by sub-category.
- **Given** a selected main category, **When** the super-admin selects a sub-category, **Then** the menu item table shows only sellable items matching both selections, shown flat.
- **Given** any category selection state, **When** the super-admin types into search, **Then** only matching items are shown (search filters items, not categories), scoped to the selected category or all items if no category is selected.
...
#### REQ-MENUMGT-008 — Price Management: default/show/happy-hour price editing with audited history · **Should** · regression

**Source:** `components/features/admin/price-update-form.tsx`, `components/features/admin/price-history-viewer.tsx`, `services/price-history-service.ts`, `app/actions/admin/price-management-actions.ts`; cross-ref REQ-102, REQ-103.
**Behaviour:** Every menu item carries three selling prices — default (`price`), show (`showPrice`), and happy-hour (`happyHourPrice`) — plus `costPerUnit`. A super-admin, or an admin with `permissions.menuManagement === true`, edits all four together on the menu item edit page's Price Management section. Saving writes one new price-history snapshot row capturing all four values (extending the existing single-row-per-change convention that already covered `price`/`costPerUnit`), and closes the previous snapshot's `effectiveTo`. Amends REQ-102 (2026-09-08, REQ-103): the save gate was previously hard-coded to `role === 'super-admin'`, contradicting the `menuManagement` permission's own UI description ("Manage menu items, categories, and pricing"); it now checks the permission.

- **Given** a super-admin or a `menuManagement`-permitted admin opens `/dashboard/menu/[id]/edit`, **When** they enter new values for default price, show price, happy-hour price, and/or cost-per-unit and save, **Then** the menu item is updated and one new price-history row is created recording all four current values.
- **Given** the price history viewer for an item, **When** it renders past snapshot rows, **Then** each row shows the default/show/happy-hour price and cost that were in effect for that period.
- **Given** a session that is neither super-admin nor holds `permissions.menuManagement === true`, **When** they attempt the update action, **Then** the request is rejected (`You do not have permission to update menu items`).

#### REQ-MENUMGT-009 — "Edit All" bulk page: view/edit price fields, name, categories, availability · **Should** · regression

**Source:** `app/dashboard/menu/edit-all/page.tsx`, `components/features/admin/menu-edit-all-table.tsx`; cross-ref REQ-102, REQ-MENUMGT-008, REQ-103.
**Behaviour:** A single page lists every menu item in a table with editable cost price, default price, show price, happy-hour price, name, main category, category, and an availability toggle, plus two filter selects (main category, then category scoped to the chosen main category) that narrow the visible rows. An "Edit All" link at the top of `/dashboard/menu` (next to "Add Item") opens this page. Saving a row (any field, price or non-price) uses the same permission gate as REQ-MENUMGT-008 — super-admin or an admin with `permissions.menuManagement === true`; price fields use the same audited price-history convention as REQ-MENUMGT-008 (one snapshot row per save); availability toggles save immediately. Amends REQ-102 (2026-09-08, REQ-103): the row-save action was previously hard-coded to `role === 'super-admin'` for the entire row (not just price fields), so a `menuManagement`-permitted admin could open the page but not save any field.

- **Given** a super-admin or a `menuManagement`-permitted admin clicks "Edit All" on `/dashboard/menu`, **When** the page loads, **Then** every menu item appears in a table with all nine editable fields above.
- **Given** the table is showing all items, **When** the admin selects a main category (and optionally a category within it), **Then** only matching items remain visible.
...
- **Given** a row's non-price fields (name, category, availability) are edited and saved, **When** the save completes, **Then** the row persists without requiring the price-history side effect.
- **Given** a session that is neither super-admin nor holds `permissions.menuManagement === true`, **When** they attempt a row save (any field), **Then** the request is rejected.

#### REQ-MENUMGT-010 — Pricing Windows: dedicated configuration page · **Should** · regression

**Source:** `app/dashboard/menu/pricing-windows/page.tsx`, `components/features/admin/pricing-windows-form.tsx`, `app/actions/admin/menu-actions.ts` (`updatePricingWindowsAction`); cross-ref REQ-102, REQ-MENUMGT-008, REQ-103. Amends REQ-102 AC2 (2026-09-05): this page replaces an earlier Settings-tab design. Amends again (2026-09-08, REQ-103): the save action moved off the generic `PUT /api/settings` (super-admin-only, and shared with unrelated settings) onto a dedicated server action gated the same way as REQ-MENUMGT-008/-009.
**Behaviour:** A super-admin, or an admin with `permissions.menuManagement === true`, independently enables/configures the "Show Price Window" and/or "Happy Hour Window" (each a single daily enabled + start/end, same convention as Settings' Business Hours) on a dedicated page at `/dashboard/menu/pricing-windows`, reached via a "Pricing Windows" button next to "Edit All" on `/dashboard/menu`. Saving persists both windows via a dedicated server action that can only ever write `showPriceWindow` / `happyHourWindow` — no other `Settings` field is reachable through this path, including Business Hours on `/dashboard/settings`.

- **Given** `/dashboard/menu`, **When** a super-admin or a `menuManagement`-permitted admin clicks "Pricing Windows", **Then** they land on `/dashboard/menu/pricing-windows` showing both the Show Price Window and Happy Hour Window sections.
- **Given** the Pricing Windows page, **When** the admin enables/configures either window and saves, **Then** the change persists and is readable back via `GET /api/settings`, and Business Hours on `/dashboard/settings` is unaffected.
...
- **Given** a session that is neither super-admin nor holds `permissions.menuManagement === true`, **When** they attempt the pricing-windows save action, **Then** the request is rejected and no `Settings` field changes.
...
#### REQ-INV-018 — Sellable inventory progressive category display · **Should** · regression

**Source:** `components/features/admin/inventory-items-client.tsx`; cross-ref REQ-082.
**Behaviour:** The sellable inventory management tab uses progressive disclosure: all sellable inventory rows are visible on landing, grouped by main category then sub-category. Main categories appear as toggle filter buttons; selecting one filters inventory rows to that category while preserving the grouped layout. Selecting a sub-category further filters rows, shown flat. Search always filters items (not categories), scoped to the selected category or all rows if no category is selected. Breadcrumb navigation shows the current category path. Kitchen ingredient COGS categories remain a separate taxonomy and are not reclassified by this requirement.

- **Given** an admin opens `/dashboard/inventory` on the sellable inventory tab, **When** the list renders, **Then** all sellable inventory rows are shown grouped by main category and sub-category, with main category toggle buttons visible.
- **Given** no main category is selected, **When** the admin selects a main category, **Then** inventory rows filter to that main category while remaining grouped by sub-category.
- **Given** a selected main and sub-category, **When** the sellable inventory list renders, **Then** only inventory rows whose linked menu item matches both selections are shown, shown flat.
...
- **Given** the pending-expense creation form, **When** an admin submits it, **Then** they must select a payment method — `cash` or `transfer` — at the group level; only cash-method groups, once transferred, reduce Current Cash Position (REQ-REPORT-007). A bank-transfer-method group's transfer confirmation requires a reference; a cash-method group's is optional (relabeled "Handed over by / notes").
- **Given** multiple pending expense groups assigned to the same payment batch, **When** the batch would mix cash-method and transfer-method groups, **Then** `assignBatch`/`confirmTransfer` reject the operation with a clear error — a batch must be payment-method-homogeneous.
...
#### REQ-FIN-006 — Expense tags · **Should** · regression

**Source:** `services/tag-service.ts`; `components/ui/tag-combobox.tsx`; `models/tag-model.ts`; cross-ref REQ-026 (pending expense groups), REQ-FIN-003

- **Given** the pending-expense creation form, **When** an admin types a new tag name into a line item's tag field and confirms, **Then** a new tag is created (case-insensitive de-duplicated by name) and attached to that line item as a selected badge.
- **Given** the same form, **When** an admin selects an existing tag from the dropdown, **Then** it is attached to that line item without creating a duplicate.
- **Given** a tag with existing expense references, **When** an admin archives it from the tag management list, **Then** it no longer appears in the create-dropdown for new line items, but expenses that already reference it continue to display it correctly (soft-delete — historical references are never invalidated).
- **Given** the expense list, **When** one or more tags are selected in the tag filter, **Then** only expenses carrying at least one of the selected tags are shown.
- **Given** a transferred pending expense group whose line items carried tags, **When** the resulting `Expense` ledger record is viewed, **Then** it retains those tags for filtering.

#### REQ-FIN-007 — Expense edit dialog completeness · **Should** · regression

**Source:** `components/features/finance/edit-expense-dialog.tsx`; `app/actions/finance/expense-actions.ts:154` (`updateExpenseAction`, super-admin only); cross-ref REQ-FIN-001, REQ-034

- **Given** the expense edit dialog, **When** a super-admin opens it for any expense, **Then** `transactionFee`, `receiptReference`, `referenceNumber`, and `linkedInventoryId` are shown as editable fields and persist correctly on save (all four already accepted by `UpdateExpenseDTO`).
...
**Source:** `services/cash-deposit-service.ts`; `app/actions/finance/cash-deposit-actions.ts`; `models/cash-deposit-model.ts`; cross-ref REQ-REPORT-007

**Behaviour:** Moving cash from the till into the company bank account follows the same `pending → approved → transferred` workflow as pending expenses, but as a distinct model — a deposit has no vendor, category, or inventory link, and never creates an `Expense` ledger row (it is not a cost). The reference field is optional (unlike a pending expense's transfer reference for bank-transfer-method groups).

- **Given** the deposits page, **When** an admin creates a deposit (date, amount, optional reference/notes), **Then** it enters the pending queue.
- **Given** a pending deposit, **When** a super-admin approves it, **Then** it moves to approved.
...
- **Given** the same menu item is sold at more than one portion size (`portionSize`: full/half/quarter) within the selected date/range, **When** the Daily or date-range report renders that item's revenue/costs rows, **Then** each portion size appears as its own row (e.g. "Peppered Meat" and "Peppered Meat (Half)"), each with the correct per-portion unit price/costPerUnit and quantity — portion sizes are never blended into one row with a weighted-average price that doesn't match any actual sale price (REQ-101). A mid-window menu price change on a _single_ portion size still sums into one row per REQ-100's per-line summation above; only a distinct `portionSize` splits the row.
...
#### REQ-REPORT-007 — Current Cash Position section + dedicated page · **Should** · regression

**Source:** `services/cash-position-service.ts`; `components/features/reports/cash-position-section.tsx`; `app/dashboard/finance/cash-position/`; `models/cash-position-adjustment-model.ts`; cross-ref REQ-FIN-003 (payment method), REQ-FIN-008 (cash deposits)

**Behaviour:** "Current Cash Position" is a live, real-time figure — the till balance as of right now — computed as opening position (carried forward from a super-admin-entered opening balance) + cash sales in − cash-method transferred expenses out − transferred cash deposits out + any adjustments recorded today = current position. It is deliberately **decoupled from any report's selected date/range**: the Daily Report's date picker never changes what this figure shows, since a section titled "Current" means now, not "as of whatever period is selected" (amended post-UAT — see `compliance/plans/REQ-106/implementation-plan.md` § "Requirements gap accepted (amended post-UAT, iteration 2)"). Computed on demand from source collections (no persisted running counter), mirroring how `paymentBreakdown`/`tipsBreakdown`/`operatingExpenses` are already computed fresh per request. Cash tips are excluded — only sale revenue (`paymentBreakdown.cash`) counts as cash in. A dedicated `/dashboard/finance/cash-position` page shows the same live balance plus a full audit ledger.

- **Given** the Daily Report, **When** it renders, **Then** a "Current Cash Position" section is always shown — including a defined empty state ("Opening balance not yet set") before any opening balance has been recorded, never omitted entirely.
- **Given** the Daily Report is viewed for any date or range (past, present, or a multi-day span), **When** the Current Cash Position section renders, **Then** it always shows the live position as of now, never the closing position or accumulated movement of whichever date/range the rest of the report is scoped to.
- **Given** a cash-method pending expense group is approved and transferred, **When** the current position is computed, **Then** it reflects the deduction from that transfer's date onward.
- **Given** a transfer-method pending expense group is approved and transferred, **When** the current position is computed, **Then** it is unaffected — only cash-method expenses touch the till.
...
#### REQ-RT-003 — Order-updated payload carries top-level status · **Must** · regression

**Source:** `lib/socket-emit-helper.ts` (`emitOrderUpdatedEvent`), `components/features/kitchen/kitchen-order-grid.tsx`, `components/features/admin/order-queue.tsx`; cross-ref REQ-083.
**Behaviour:** The `order:updated` socket payload carries a top-level `status` field (not only nested in `updates.status`). The kitchen display grid uses this top-level status to update the order in-place. When an order transitions to a terminal status (completed, cancelled), the order is removed from the kitchen display store rather than updated — preventing a `router.refresh()` race that previously reverted the status to its pre-terminal value. The admin order queue subscribes to `order:updated` events for real-time status synchronisation.

- **Given** an order in a non-terminal status on the kitchen display, **When** a `order:updated` event arrives, **Then** the grid updates the order's status in-place using the top-level `status` field without a full page reload.
- **Given** an order marked completed or cancelled, **When** the `order:updated` event arrives, **Then** the order is removed from the kitchen display store immediately and does not reappear.
- **Given** the admin order queue page is open, **When** an order's status changes server-side, **Then** the queue updates in real-time via socket subscription without a manual refresh.
...
> ⚠ **Resolved (#117 P0#1):** webhook handlers now have an idempotency guard via `lib/webhook-idempotency.ts` `recordWebhookEvent()` — replayed events are deduplicated and do not re-deduct inventory or re-grant points/rewards. See REQ-PAY-002.
...
## Feature Area 27 — WhatsApp Comms (WA)

**Fixtures/env:** Ability to read issued PINs (DB seed/test hook) since WhatsApp delivery is stubbed at the boundary; a customer with `whatsappTransactional: true` and `whatsappMarketing: false` (defaults); an order for transactional template tests.

#### REQ-WA-001 — OTP delivery via WhatsApp · **Must** · regression

**Source:** `lib/notification-templates.ts` (`verification_pin` → `authentication`), `services/notification-service.ts`; cross-ref REQ-053/054.
**Behaviour:** The `verification_pin` template is categorized as `authentication` in `TEMPLATE_CATEGORIES`. The `NotificationService.send()` orchestrator skips the consent check entirely for `authentication`-category templates (Meta exempts AUTHENTICATION templates from consent). When a PIN is issued, the WhatsApp attempt is made without gating on `whatsappTransactional` or `whatsappMarketing`.

- **Given** a user requests a PIN, **When** `NotificationService.send` dispatches the `verification_pin` template, **Then** the WhatsApp attempt is made without checking any consent flag (authentication category is exempt).
- **Given** a user with `whatsappTransactional: false`, **When** a PIN is issued, **Then** the WhatsApp OTP attempt still fires because authentication templates bypass the consent gate.

#### REQ-WA-002 — Transactional WhatsApp templates · **Should** · regression

**Source:** `lib/notification-templates.ts` (`order_confirmation`, `order_status_update`, `receipt`, `payment_link`, `payment_confirmation`, `bank_transfer_details` → `transactional`), `services/notification-service.ts`; cross-ref REQ-054/062.
**Behaviour:** Transactional templates (order confirmation, status update, receipt, payment link, payment confirmation, bank transfer details) are categorized as `transactional` in `TEMPLATE_CATEGORIES`. The orchestrator gates the WhatsApp attempt on `whatsappTransactional === true` (default true, captured at first PIN verification per REQ-053). If the user has not opted in to transactional WhatsApp, the notification falls through to email.

- **Given** a user with `whatsappTransactional: true`, **When** an order confirmation is sent, **Then** the WhatsApp `order_confirmation` template is attempted.
- **Given** a user with `whatsappTransactional: false`, **When** an order confirmation is sent, **Then** the WhatsApp attempt is skipped and the notification falls through to email.
- **Given** a payment confirmation event, **When** `NotificationService.send` dispatches the `payment_confirmation` template, **Then** the `whatsappTransactional` consent flag is checked before the WhatsApp attempt.

#### REQ-WA-003 — Support reply via WhatsApp · **Should** · regression

**Source:** `lib/notification-templates.ts` (`support_reply` → `transactional`), `services/notification-service.ts`; cross-ref REQ-064.
**Behaviour:** The `support_reply` template is categorized as `transactional`. When staff reply to a support ticket, `NotificationService.send` attempts the WhatsApp `support_reply` template gated on `whatsappTransactional`. Outbound `support_reply` WhatsApp sends fall through to email until Meta approves the template (WA-1 thread). The ticket is the source of truth regardless of delivery channel.

- **Given** a staff reply to a support ticket, **When** `NotificationService.send` dispatches `support_reply`, **Then** the `whatsappTransactional` consent flag is checked and the WhatsApp attempt is made if opted in.
- **Given** Meta has not yet approved the `support_reply` template, **When** the WhatsApp attempt fails, **Then** the notification falls through to email and the ticket reply is still recorded.

#### REQ-WA-004 — Marketing WhatsApp templates with consent gate · **Should** · regression

**Source:** `lib/notification-templates.ts` (`reward_earned`, `reward_expiring_soon` → `marketing`), `services/notification-service.ts`; cross-ref REQ-063.
**Behaviour:** Marketing templates (reward earned, reward expiring soon) are categorized as `marketing` in `TEMPLATE_CATEGORIES`. The orchestrator gates the WhatsApp attempt on `whatsappMarketing === true` (default false, requires explicit opt-in per REQ-063). If the user has not opted in to marketing WhatsApp, the notification falls through to email if `emailMarketing === true`, or is skipped entirely.

- **Given** a user with `whatsappMarketing: true`, **When** a reward is earned, **Then** the WhatsApp `reward_earned` template is attempted.
- **Given** a user with `whatsappMarketing: false` (default), **When** a reward is earned, **Then** the WhatsApp attempt is skipped and the notification falls through to email (if `emailMarketing: true`) or is not sent.
- **Given** a user with `whatsappMarketing: false`, **When** a reward is expiring soon, **Then** the WhatsApp `reward_expiring_soon` template is not attempted.

#### REQ-WA-005 — Inbound WhatsApp message handling · **Should** · regression

**Source:** `services/whatsapp-inbound-service.ts`, `app/api/webhooks/whatsapp/route.ts`, `lib/whatsapp.ts`; cross-ref REQ-056.
**Behaviour:** Inbound WhatsApp messages are received via the webhook at `/api/webhooks/whatsapp` and routed through `WhatsAppInboundService.handle`. The service classifies the sender's customer state (`new`, `signing_up`, `active`, `dormant`) and the message intent (`opt_out`, `chat_with_staff`, `support_text`). STOP/opt-out messages matching `/^\s*(stop|unsubscribe|opt[-\s]?out)\s*$/i` clear both `whatsappTransactional` and `whatsappMarketing` regardless of customer state (Meta WABA policy). New/signing-up users receive a `welcome_new_user` template; dormant users receive `welcome_back`. Active users are queued for staff. Support-text intents create a `SupportTicket` with `source: whatsapp`. An `IncomingMessage` audit record is persisted for every inbound message.

- **Given** an inbound WhatsApp message from an unknown phone number, **When** `WhatsAppInboundService.handle` processes it, **Then** a new `User` row is auto-created and a `welcome_new_user` template is sent.
- **Given** an inbound message matching the STOP opt-out pattern, **When** the service processes it, **Then** both `whatsappTransactional` and `whatsappMarketing` are set to false and a free-form STOP confirmation is sent within the 24h customer-service window.
- **Given** an inbound support-text message from an active user, **When** the service processes it, **Then** a `SupportTicket` is created with `source: whatsapp` and the message body as the ticket subject.
- **Given** any inbound message, **When** the service processes it, **Then** an `IncomingMessage` audit record is persisted.

#### REQ-WA-006 — Consent model: transactional default on, marketing default off · **Must** · regression

**Source:** `services/notification-service.ts`, `lib/notification-templates.ts`; cross-ref REQ-053/063.
**Behaviour:** The WhatsApp consent model has two independent flags: `whatsappTransactional` (default `true`, captured at first PIN verification per REQ-053) and `whatsappMarketing` (default `false`, requires explicit opt-in per REQ-063). `NotificationService.send()` reads the template's category from `TEMPLATE_CATEGORIES` and checks the corresponding consent flag before attempting the WhatsApp send. Authentication-category templates bypass the consent check entirely. The `communicationPreferencesUpdatedAt` timestamp is stamped on any consent change.

- **Given** a new user completing first PIN verification, **When** the consent flags are persisted, **Then** `whatsappTransactional` defaults to `true` and `whatsappMarketing` defaults to `false`.
- **Given** a `transactional`-category template, **When** `NotificationService.send` is called, **Then** the `whatsappTransactional` flag is checked; if false, the WhatsApp attempt is skipped.
- **Given** a `marketing`-category template, **When** `NotificationService.send` is called, **Then** the `whatsappMarketing` flag is checked; if false, the WhatsApp attempt is skipped.
- **Given** an `authentication`-category template, **When** `NotificationService.send` is called, **Then** no consent flag is checked and the WhatsApp attempt proceeds unconditionally.
