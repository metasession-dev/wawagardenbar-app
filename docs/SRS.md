...
| REQ-MENUMGT-007  | Menu management item lists use main-category cascade and contextual search          | Should   | regression | `components/features/admin/menu-items-client.tsx`; `menu-item-form.tsx`; `menu-item-edit-form.tsx`; REQ-081      |
...
| REQ-INV-018      | Sellable inventory list uses menu main-category cascade and contextual search       | Should   | regression | `components/features/admin/inventory-items-client.tsx`; REQ-081                                                  |
...
**Behaviour:** Express order item selection starts with enabled configured main menu categories, then shows only sub-categories for the selected main, then shows only sellable `kind:'menu-item'` items matching both selections. Search stays available throughout the flow, filtering the current stage or the selected item set without bypassing the main-category -> sub-category navigation. The cart/order context is preserved while staff navigate back from items to sub-categories and from sub-categories to main categories, so one order can include items from multiple mains.
...
- **Given** the express flow is at the main-category, sub-category, or selected-item stage, **When** staff types into search, **Then** the current stage remains intact and only matching categories or items for that stage are shown.
...
**Behaviour:** Admin menu-management item browsing starts from the configured main-category registry, then narrows to sub-categories under the selected main. Search stays available and filters the current category stage or the selected item table without bypassing the cascade. Create and edit forms keep the existing main-category-before-sub-category flow and clear an invalid sub-category value when the selected main category changes.
...
- **Given** the menu-management flow is at the main-category, sub-category, or selected-item stage, **When** the super-admin types into search, **Then** only matching categories or rows for that stage are shown and the cascade state is preserved.
...
**Behaviour:** The sellable inventory management tab uses the same configured main-category -> sub-category cascade as menu management when filtering inventory rows backed by menu items. Search stays available and filters the current category stage or the selected inventory rows without bypassing the cascade. Kitchen ingredient COGS categories remain a separate taxonomy and are not reclassified by this requirement.
...
- **Given** the sellable-inventory flow is at the main-category, sub-category, or selected-item stage, **When** the admin types into search, **Then** only matching categories or rows for that stage are shown and the cascade state is preserved.
