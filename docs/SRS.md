...
**Source:** `components/features/admin/price-update-form.tsx`, `components/features/admin/price-history-viewer.tsx`, `services/price-history-service.ts`, `app/actions/admin/price-management-actions.ts`; cross-ref REQ-102.
**Behaviour:** Every menu item carries three selling prices — default (`price`), show (`showPrice`), and happy-hour (`happyHourPrice`) — plus `costPerUnit`. A super-admin edits all four together on the menu item edit page's Price Management section. Saving writes one new price-history snapshot row capturing all four values (extending the existing single-row-per-change convention that already covered `price`/`costPerUnit`), and closes the previous snapshot's `effectiveTo`.

- **Given** a super-admin opens `/dashboard/menu/[id]/edit`, **When** they enter new values for default price, show price, happy-hour price, and/or cost-per-unit and save, **Then** the menu item is updated and one new price-history row is created recording all four current values.
...
- **Given** a non-super-admin session, **When** they attempt the update action, **Then** the request is rejected (`Only super-admin can update prices`), matching the existing single-price RBAC gate.

#### REQ-MENUMGT-009 — "Edit All" bulk page: view/edit price fields, name, categories, availability · **Should** · regression

**Source:** `app/dashboard/menu/edit-all/page.tsx`, `components/features/admin/menu-edit-all-table.tsx`; cross-ref REQ-102, REQ-MENUMGT-008.
**Behaviour:** A single page lists every menu item in a table with editable cost price, default price, show price, happy-hour price, name, main category, category, and an availability toggle, plus two filter selects (main category, then category scoped to the chosen main category) that narrow the visible rows. An "Edit All" link at the top of `/dashboard/menu` (next to "Add Item") opens this page. Saving a row's price fields uses the same audited price-history convention as REQ-MENUMGT-008 (one snapshot row per save); availability toggles save immediately.

- **Given** a super-admin clicks "Edit All" on `/dashboard/menu`, **When** the page loads, **Then** every menu item appears in a table with all nine editable fields above.
