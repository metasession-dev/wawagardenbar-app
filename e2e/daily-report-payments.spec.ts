...
test.describe.serial(
  'REQ-013: Daily Report — Partial Payment & Tab Payment Accuracy',
  () => {
    ...
    test('create tab and add order', async ({ page }) => {
      ...
      if (!(await phoneInput.inputValue()))
        await phoneInput.fill('08099887766');
    ...
    test('close tab with final payment (card)', async ({ page }) => {
      ...
      await dialog
        .getByRole('button', { name: /Pay.*Close Tab/i })
        .click();
      await expect(
        page.getByText(/Tab payment completed/i).first()
      ).toBeVisible({ timeout: 10000 });
    });
    ...
    test('daily report reflects both payment methods with no double-counting', async ({
      ...
      await expect(
        page.getByText('POS / Card', { exact: true })
      ).toBeVisible();
...
// ═══════════════════════════════════════════════════════════════════════════
test.describe.serial(
  'REQ-013: Partial payment on open tab appears in daily report',
  () => {
    let baseline: Awaited<ReturnType<typeof getReportAmounts>>;
    let openTabId = '';
    let openTabTotal = 0;
    ...
    test('create tab, add order, record partial payment', async ({ page }) => {
      ...
      // Read tab total
      const descText =
        (await dialog.locator('p').first().textContent()) ?? '';
      ...
      await dialog
        .locator('#partialNote')
        .fill('E2E open-tab partial payment');
      await dialog.locator('#partial-cash').click();
      await dialog
        .getByRole('button', { name: /Record Partial Payment/i })
        .click();
