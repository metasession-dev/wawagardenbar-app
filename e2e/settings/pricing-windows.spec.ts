/**
 * @requirement REQ-102 — AC2: Show Price Window / Happy Hour Window settings
 *
 * Covers AC2 from compliance/evidence/REQ-102/test-scope.md:
 *   Given a super-admin opens /dashboard/settings, When they view the new
 *   "Show Price Window" and "Happy Hour Window" sections, Then they can
 *   independently enable/disable each window and set its daily start/end
 *   time, and saving persists both without affecting Business Hours.
 *
...
async function gotoPricingWindowsTab(page: Page) {
  await page.goto('/dashboard/settings', { waitUntil: 'domcontentloaded' });
  await page.getByRole('tab', { name: 'Pricing Windows' }).click();
}

superAdminTest.describe.configure({ mode: 'serial' });

superAdminTest.describe(
  'REQ-102: Settings — Show Price Window / Happy Hour Window',
  () => {
    superAdminTest(
      'AC2: Pricing Windows tab shows both window sections',
      async ({ page }) => {
        tagTest('REQ-102', 2);
        await gotoPricingWindowsTab(page);

        await expect(
          page.getByTestId('show-price-window-heading')
        ).toBeVisible();
        await expect(
          page.getByTestId('happy-hour-window-heading')
        ).toBeVisible();
        await evidenceShot(page, 'REQ-102', 2, 'pricing-windows-tab', {
          tier: 'feature',
        });
      }
    );
    ...
    superAdminTest(
      ...
      async ({ page }) => {
        ...
        // grandparent `div.space-y-4`) so the Show Price Window's switch,
        // which sits in a sibling card, is never touched.
        ...
        await expect(
          page
            .getByText('Settings updated successfully', { exact: true })
            .first()
        ).toBeVisible({
          timeout: 10000,
        });
    ...
    superAdminTest(
      'AC2: Business Hours section is unaffected by the new Pricing Windows tab',
      async ({ page }) => {
        tagTest('REQ-102', 2);
